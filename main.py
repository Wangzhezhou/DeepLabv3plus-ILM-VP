...
from utils.visual_prompt import ExpansiveVisualPrompt
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            # Interpolate the outputs to match the size of the labels
...
def main():
    ...
    visual_prompt = ExpansiveVisualPrompt(out_size=224, mask=np.zeros((224, 224)), init='zero', normalize=None)
    model = nn.Sequential(visual_prompt, model)

    if opts.separable_conv and 'plus' in opts.model:
        network.convert_to_separable_conv(model.classifier)
    utils.set_bn_momentum(model[1].backbone, momentum=0.01)
    ...
    while cur_itrs < total_itrs_for_75f_epochs:  # Instead of training from stratch
        ...
        model.train()
        cur_epochs += 1
        if cur_epochs % mapping_interval == 0:
            mapping_sequence = generate_label_mapping_by_frequency(visual_prompt, model, train_loader)
            label_mapping = partial(label_mapping_base, mapping_sequence=mapping_sequence)
        ...
        for (images, labels) in train_loader:
            ...
            outputs = label_mapping(model(images))
            outputs = torch.nn.functional.interpolate(outputs, size=(513, 513), mode='bilinear', align_corners=True)
            ...
            adjusted_val_interval = opts.val_interval // 2
            if (cur_itrs) % adjusted_val_interval == 0:
